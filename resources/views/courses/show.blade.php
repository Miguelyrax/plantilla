<x-app-layout>
    <section class="bg-gray-700 py-12 mb-12">
        <div class="container grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <p> <i class="far fa-star"></i> Calificaión: {{$course->rating}}</p>
            </div>
        </div>
    </section>
    <div class="container grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="order-2 lg:col-span-2 lg:order-1">
            ...
            @if ($loop->first) {{--Si nos encontramos en la primera iteracion, esta condicional va a dar verdadero--}}
                x-data="{open: true}"
                @else
                x-data="{open: false}"
            @endif >
                        ...
                        <div class="bg-white py-2 px-4 " x-show="open">
                            <ul class="grid grid-cols-1 gap-2">{{--Solucion a poner margin boton, ya que se descuadra--}}
                                @foreach ($section->lessons as $lesson)
                            <li class="text-gray-700 text-base "> <i class="fa fa-play-circle mr-2 text-gray-600"></i> {{$lesson->name}}</li>
                                @endforeach
                            </ul>
        ...
        <div class="order-1 lg:order-2">
            <section class="card mb-4 ">
                ...
                <form action="{{route('course.enrolled', $course)}}" method="post">
                    @csrf
                        <button class="btn btn-danger btn-block mt-4" submit>Llevar este curso</button>
